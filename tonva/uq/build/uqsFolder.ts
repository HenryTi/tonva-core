...
export async function buildUqsFolder(tonva:Tonva, uqsFolder:string/*, appConfig: AppConfig*/) {
	let uqErrors = await uqsStart(tonva/*, appConfig*/);
...
// 返回每个uq构建时的错误
async function uqsStart(tonva:Tonva/*, uqsConfig: AppConfig*/):Promise<string[]> {
	env.isDevelopment = true;
	await tonva.nav.init();
	let retErrors = await tonva.uqsMan.buildUQs();
	return retErrors;
}
